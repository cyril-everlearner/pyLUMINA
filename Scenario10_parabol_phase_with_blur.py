...
The default values shows a parabolic phase applied with an SLM and a blur mimicking the SLM resolution limits.
A line shaped beam is observed with additional diffraction du to the blur.
...
def run_simulation():
    ...
    try:
        ...
        # Applying parabolic phase
        if apply_parabolic_phase_var.get():
            Second_order_coeff = float(entry_2nd_order_coef.get())
            source = apply_parabolic_phase(source, taillefenetre, nbpixel, Second_order_coeff)
...
# Champ pour la phase parabolique
ttk.Label(frame, text="Parabolic phase (rad):").grid(row=len(parameters) + 2, column=0, sticky=tk.W)
entry_2nd_order_coef = ttk.Entry(frame, width=15)
entry_2nd_order_coef.grid(row=len(parameters) + 2, column=1)
entry_2nd_order_coef.insert(0, "3000")  # Valeur par défaut

# Checkbox pour activer/désactiver la phase parabolique
apply_parabolic_phase_var = tk.BooleanVar(value=True)
checkbox_2nd_order_coef = ttk.Checkbutton(frame, text="Apply Parabolic Phase?", variable=apply_parabolic_phase_var)
checkbox_2nd_order_coef.grid(row=len(parameters) + 2, column=2, sticky=tk.W)
...
checkbox_blur.grid(row=len(parameters) + 3, column=2, sticky=tk.W)

# Blur sigma value (gaussian filter)
ttk.Label(frame, text="Sigma (pixels):").grid(row=len(parameters) + 3, column=0, sticky=tk.W)
entry_blur_sigma = ttk.Entry(frame, width=15)
entry_blur_sigma.grid(row=len(parameters) + 3, column=1)
entry_blur_sigma.insert(0, ".5")  # Valeur par défaut
